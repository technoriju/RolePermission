                        ...
                        <div class="card-body">
                            ...
                            <form class="forms-sample" action="{{route('user.update',$user->id)}}" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="phone" class="form-control" id="exampleInputUsername1" placeholder="Phone" value="{{old('phone',$user->phone)}}">
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputUsername1">Email</label>
                                    <input type="email" name="email" class="form-control" id="exampleInputUsername1" placeholder="Email" value="{{old('email',$user->email)}}">
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    @if(isset($role) && count($role)>0)
                                        @foreach($role as $val)
                                        <option value="{{$val->name}}" {{(in_array($val->id,$roles))?'selected': ''}}>{{$val->name}}</option>
                                        @endforeach
                                    @endif
                                    </select>
                                </div>
                                    ...
                                    <div class="row">
                                      <div class="form-check">
                                        <label class="form-check-label">
                                          <input type="radio" class="form-check-input" name="status" id="membershipRadios1" value="{{ old('status',1)}}" {{$user->status == 1 ? 'checked' : ''}}>
                                          Active
                                        </label>
                                      </div>
                                      <div class="form-check">
                                        <label class="form-check-label">
                                          <input type="radio" class="form-check-input" name="status" id="membershipRadios2" value="{{ old('status',0)}}" {{$user->status == 0 ? 'checked' : ''}}>
                                           Inactive
                                        </label>
                                      </div>
                                    </div>
